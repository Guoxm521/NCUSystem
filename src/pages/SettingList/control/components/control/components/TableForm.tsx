...
const TableForm: FC<TableFormProps> = ({ value, onChange }) => {
  ...
  const saveRow = (e: React.MouseEvent | React.KeyboardEvent, key: string) => {
    ...
    setTimeout(() => {
      ...
      if (!target.icon || !target.number || !target.appName || !target.appType || !target.appPath) {
        ...
        if (record.editable) {
          return (
            <Input
              value={text}
              autoFocus
              onChange={(e) => handleFieldChange(e, 'icon', record.key)}
        ...
        if (record.editable) {
          return (
            <Input
              value={text}
              autoFocus
              onChange={(e) => handleFieldChange(e, 'number', record.key)}
              onKeyPress={(e) => handleKeyPress(e, record.key)}
              placeholder="编号"
            />
          );
      ...
      render: (text: string, record: TableFormDateType) => {
        if (record.editable) {
          return (
            <Input
              value={text}
              onChange={(e) => handleFieldChange(e, 'appName', record.key)}
              onKeyPress={(e) => handleKeyPress(e, record.key)}
              placeholder="应用名称"
            />
          );
      ...
      render: (text: string, record: TableFormDateType) => {
        if (record.editable) {
          return (
            <Input
              value={text}
              onChange={(e) => handleFieldChange(e, 'appType', record.key)}
              onKeyPress={(e) => handleKeyPress(e, record.key)}
              placeholder="应用类别"
            />
          );
      ...
      render: (text: string, record: TableFormDateType) => {
        if (record.editable) {
          return (
            <Input
              value={text}
              onChange={(e) => handleFieldChange(e, 'appPath', record.key)}
              onKeyPress={(e) => handleKeyPress(e, record.key)}
              placeholder="连接地址"
            />
          );
